...
# Module for clustering isolated phase domains. Module called within inputscript_clustered.py
# Do not run this script, change parameters and file names in inputscript_clustered.py and run using ...
# ... python/python3 inputscript_clustered.py
...
def clustered_domains(params, file_names):
    ...
    for iter_dom in range(num_domains - 1):
        for next_dom in range(iter_dom + 1, num_domains):

            diff_thet = np.absolute(ori_domains_thet[geometric_centers[next_dom], next_dom] - ori_domains_thet[geometric_centers[iter_dom], iter_dom])
            diff_phi = np.absolute(ori_domains_phi[geometric_centers[next_dom], next_dom] - ori_domains_phi[geometric_centers[iter_dom], iter_dom])

            if (diff_phi >= num_bin_phi/2):

                diff_phi = num_bin_phi - 1 - diff_phi

            ini_dist_dom_cent += diff_thet**2.0 + diff_phi**2.0
                    ...
                    if(grid_succ):
                        ...
                        for iter_dom in range(num_domains - 1):
                            for next_dom in range(iter_dom + 1, num_domains):
                                ...
                                diff_thet = np.absolute(temp_domains_thet[geometric_centers[next_dom], next_dom] - temp_domains_thet[geometric_centers[iter_dom], iter_dom])
                                diff_phi = np.absolute(temp_domains_phi[geometric_centers[next_dom], next_dom] - temp_domains_phi[geometric_centers[iter_dom], iter_dom])

                                if (diff_phi >= num_bin_phi/2):

                                    diff_phi = num_bin_phi - 1 - diff_phi

                                loop_dist_dom_cent += diff_thet**2.0 + diff_phi**2.0

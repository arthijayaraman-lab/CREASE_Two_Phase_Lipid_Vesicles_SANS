...
def evenly_spaced_domains(params, file_names):
    ...
    if (max_diff == 0):
        ...
        dist_dom_phi = (max_diff)/((num_bin_phi)*(num_domains - 1)/num_domains)

        if (dist_dom_phi > 1.0):
            dist_dom_phi = 1 - (dist_dom_phi - 1)         
	
        ini_dist_dom_phi = dist_dom_phi + (1 - np.std(dist_between_doms_phi)/(num_bin_phi*(num_domains - 1)/num_domains))
                ...
                for phi in range(num_bin_phi):
                    ...
                    if(grid_succ):
                        ...
                        if (max_diff == 0):
                            ...
                            loop_dom_phi_max = max_diff/(num_bin_phi*(num_domains - 1)/num_domains)
                            if (loop_dom_phi_max > 1.0):
                                loop_dom_phi_max = 1 - (loop_dom_phi_max - 1)

                            loop_dist_dom_phi = loop_dom_phi_max + (1 - np.std(dist_between_doms_phi)/(num_bin_phi*(num_domains - 1)/num_domains))
